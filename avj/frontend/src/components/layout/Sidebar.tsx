...
const NAV_ITEMS = [
  ...
  { id: 'me' as NavId,     label: "Mening profil",path: '/profile',     icon: 'user' as const },
];
  ...
  return (
    ...
    >
      ...
      {ME.now && (
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <Album name={ME.now.album} artist={ME.now.artist} size={40} radius={8} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: -0.2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{ME.now.song}</div>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{ME.now.artist}</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingLeft: 4 }}>
        <Avatar name={ME.name} size={36} live />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 600 }}>{ME.name.split(' ')[0]}</div>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>{ME.handle}</div>
        </div>
        <button
          onClick={toggleTheme}
          style={{
